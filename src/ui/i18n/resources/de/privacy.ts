/** PrivacyView-specific copy. `sections` is read back via t('sections', { returnObjects: true })
 * rather than split into per-section flat keys - it's naturally an ordered list of {title, text}
 * pairs, and i18next supports returning arrays/objects from a single key for exactly this shape. */
const privacy = {
  ...
  sections: [
    ...
    },
    {
      ...
      text: 'Nein. Die App bindet keine Analyse- oder Tracking-Dienste ein. Auch die Schriftart wird nicht nachgeladen, sondern ist Teil der App selbst, damit kein Schriftanbieter erfährt, wann du sie benutzt. Der Anmeldecode von Google wird erst in dem Moment geladen, in dem du auf „Mit Google anmelden“ klickst. Die einzigen drei Ausnahmen, bei denen die App von sich aus eine Verbindung zu einem fremden Server aufbaut: dieser Google-Anmeldecode, ein einmaliger stiller Verbindungsversuch zu Google unmittelbar nach einem Neuladen der App, das die App selbst infolge eines Datenimports ausgelöst hat, und das Laden der Seite „Änderungen“, die die öffentlichen, anonymen Versionshinweise dieses Projekts direkt von GitHub anzeigt (ohne Zugangsdaten, ohne dass dabei irgendwelche deiner Daten übertragen werden). Außerhalb dieser drei Fälle nimmt die App zu keinem fremden Server Verbindung auf.',
    },
  ],
} as const;
