...
- **Selection and right-click share one list**, `app/selectableText.ts`. Released today: form
  fields, anything with `role="alert"`, anything with the `.MuiAlert-root` class (which is what
  keeps a SUCCESS notification copyable - those carry `role="status"`, not `alert`), and
...
## Rückmeldungen und Ladestatus

**One mechanism, one place.** `app/store/notificationStore.ts` holds a queue,
`app/AppNotifications.tsx` renders it as a single Snackbar, mounted from `App.tsx` next to
`UpdatePrompt` - and for the same reason: the print route lives outside `AppShell`. Anywhere in the
app, `notify.success(text)` / `notify.error(text)` / `notify.report(e, context)` is how feedback is
raised. `notify.report` reproduces the old `useErrorSnackbar.report` formatting exactly, which is
why no German string changed when the three previous mechanisms were merged into this one.

Rules that cost something to relearn:

- **One atomic Zustand selector per value.** Zustand 5 has no built-in shallow compare, so a
  selector returning a fresh object or array (`s => ({ current: s.queue[0], dismiss: s.dismiss })`)
  loops on React's "getSnapshot should be cached".
- **`queue[0]` is possibly `undefined` and `tsconfig` has no `noUncheckedIndexedAccess`.** It types
  clean and throws at runtime. The host is mounted above the router, so that is a white screen on
  every route including `/print`.
- **Errors auto-hide too**, they just get longer (10 s versus 4 s). A Snackbar sits at z-index 1400,
  above every Dialog, and is full width below 600 px; a permanent one would cover the Speichern
  button of the very dialog that raised the error.
- **No de-duplication.** `ScheduleView` gives every cell write the same context string, so two cells
  failing from one cause produce byte-identical text - dropping the second would leave a lost write
  with no feedback at all.
- Any test that renders a view which reports something must mount `<AppNotifications />` too, and
  clear the store in `beforeEach`: it is a module singleton and leaks between tests.

**Every wait after a click shows something.** The rule that goes with it: no `setBusy(true)` without
a `catch` that reports and a `finally` that clears it. A busy flag without a `finally` turns a silent
failure into a permanently frozen dialog - `exportData` and `deleteAllData` had no `try/catch` at all
before this. `components/ConfirmDialog.tsx` takes an optional `busy` that disables both buttons,
spins the confirm button and short-circuits `onClose`, which covers the import, the delete-all and
the Drive delete in one place. Two initial loads get an indicator as well, because their old
behaviour actively misled: `PrintPreviewView` rendered a blank white page, and `ScheduleView` used to
drop its whole table on a week switch. That overlay covers the KPI tiles as well as the table, and
disables `ScheduleHeaderFields` while it runs - those fields save on blur, so leaving one mid-switch
would write the old week back.
