...
describe('CarryOverPreviousWeekDialog', () => {
  ...
  it('prefills 0 when targetAdjustmentMinutes was explicitly stored as 0, instead of recomputing a fresh suggestion', async () => {
    const e5 = 'e5' as EmployeeId;
    const emp = employee(e5, { lastName: 'Fünf', firstName: 'Frida', employmentType: { type: 'FullTime', weeklyHours: 15 } });
    // Same fixture as the non-zero case above: Vorwoche komplett Off -> a freshly computed
    // suggestion would be +15 Std., which must NOT reappear once the user has deliberately zeroed
    // the adjustment - a stored 0 means "transfer nothing", not "nothing was ever stored".
    findForWeekMock.mockResolvedValue(createWeeklySchedule(branchId, previousWeek, [e5]));
    const currentSchedule = withTargetAdjustment(createWeeklySchedule(branchId, currentWeek, [e5]), e5, 0);
    ...
    const cells = within(row).getAllByRole('cell');
    expect(cells[3].textContent).toBe('+15');
    expect(inputFor('Fünf, Frida')).toHaveValue('0');
  });
